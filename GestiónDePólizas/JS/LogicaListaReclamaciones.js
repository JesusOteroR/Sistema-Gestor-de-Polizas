  ...
  function listarReclamos(pagina, busqueda = "") {
    fetch(`http://www.polizaswebapp.somee.com/api/Reclamo`)
      ...
      .then((data) => {
        ...
        if (busqueda) {
          data = data.filter((reclamo) =>
            reclamo.Numero.toString().includes(busqueda)
          );
        }
        ...
        reclamosPagina.forEach((reclamo) => {
          ...
          row.innerHTML = `
            <td><input type="checkbox" name="option" value="${
              reclamo.Numero
            }"></td>
  ...
  function actualizarPaginacion(totalPaginas) {
    ...
    botonAnterior.addEventListener("click", () => {
      if (paginaActual > 1) {
        ...
        listarReclamos(
          paginaActual,
          document.getElementById("busquedaNumero").value
        );
      }
    });
    ...
    Swal.fire({
      ...
      preConfirm: () => {
        ...
        const descripcion = document
          .getElementById("descripcionReclamo")
          .value.trim();
        ...
        .then((reclamo) => {
          ...
          Swal.fire({
            ...
            preConfirm: () => {
              ...
              return fetch(
                `http://www.polizaswebapp.somee.com/api/Reclamo/${numeroReclamo}`,
                {
                  method: "PUT",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify(data),
    ...
    if (selected) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          fetch(`http://www.polizaswebapp.somee.com/api/Reclamo/${reclamoId}`, {
            method: "DELETE",
          })
            .then((response) => {
            ...
            .then(() => {
              Swal.fire({
                title: "Eliminado",
                text: "Reclamo eliminado exitosamente.",
                icon: "success",
              });
              listarReclamos();
            })
      ...
      Swal.fire({
        title: "Error",
        text: "No has seleccionado ningún reclamo para eliminar.",
        icon: "error",
      });
    }
  }
