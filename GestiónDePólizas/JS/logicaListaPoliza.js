document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderTable() {
    ...
    paginatedPolizas.forEach((poliza) => {
      ...
      const aseguradora = aseguradorasMap[poliza.id_Aseguradora] || {
        Nombre: "Desconocido",
      };
  ...
  // Función para filtrar pólizas por número o aseguradora
  function buscarPoliza() {
    ...
    // Filtrar pólizas
    filteredPolizas = polizas.filter((poliza) => {
      // Comprobar si el número de póliza incluye el input de búsqueda
      const matchesPoliza = poliza.id_Poliza.toString().includes(searchInput);

      // Obtener el nombre de la aseguradora del mapa usando su ID
      const aseguradoraNombre = aseguradorasMap[poliza.id_Aseguradora]
        ? aseguradorasMap[poliza.id_Aseguradora].Nombre.toLowerCase()
        : "";

      // Comprobar si el nombre de la aseguradora incluye el input de búsqueda
      const matchesAseguradora = aseguradoraNombre.includes(searchInput);

      // Retorna true si coincide con alguna de las dos condiciones
      return matchesPoliza || matchesAseguradora;
    });
  ...
  document
    ...
    .addEventListener("click", buscarPoliza);

  // Configurar la búsqueda en tiempo real
  document
    .getElementById("searchInput")
    .addEventListener("input", buscarPoliza);

  // Función para listar las aseguradoras
      ...
      fetch(`http://www.polizaswebapp.somee.com/api/Poliza/${polizaId}`)
        ...
        .then((poliza) => {
          ...
          Swal.fire({
            ...
            preConfirm: () => {
              const modificarAseguradora = document
                .getElementById("companiaSeguro")
                .value.trim();
              const modificarTipo = document
                .getElementById("modificarTipo")
                .value.trim();
              const modificarCobertura = document
                .getElementById("modificarCobertura")
                .value.trim();
              const modificarFechaInicio = document
                .getElementById("modificarFechaInicio")
                .value.trim();
              const modificarFechaVencimiento = document
                .getElementById("modificarFechaVencimiento")
                .value.trim();
              const modificarDescripcion = document
                .getElementById("modificarDescripcion")
                .value.trim();
              const modificarUsuario = document
                .getElementById("modificarUsuario")
                .value.trim();
              ...
              return fetch(
                `http://www.polizaswebapp.somee.com/api/Poliza/${polizaId}`,
                {
                  method: "PUT",
                  headers: {
  ...
  document
    ...
    .addEventListener("click", eliminarPoliza);
